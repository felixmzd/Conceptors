...
class ReservoirBinocular:
    def __init__(
        ...
        self.trust_smooth_rate = trust_smooth_rate
        self.trust_adapt_steepness12 = trust_adapt_steepness12
        self.trust_adapt_steepness23 = trust_adapt_steepness23
        self.hypo_adapt_rate = hypo_adapt_rate
        self.drift = drift
    ...
    def fit(self, patterns, t_learn=600, t_learnc=2000, t_wash=200):
        ...
        for i, p in enumerate(self.patterns):
            ...
            self.all_rand["r"][:, i * self.t_learn : (i + 1) * self.t_learn] = rando[
                "r"
            ]
            self.all_rand["p"][:, i * self.t_learn : (i + 1) * self.t_learn] = rando[
                "p"
            ]
            self.all_rand["z"][:, i * self.t_learn : (i + 1) * self.t_learn] = rando[
                "z"
            ]
            self.all_rand["old_cphi"][
                :, i * self.t_learn : (i + 1) * self.t_learn
            ] = rando["old_cphi"]

        # recompute G
        features = self.all_rand["old_cphi"]
        targets = self.all_rand["z"]
        self.G = utils.RidgeWload(features, targets, self.TyA_G)
        ...
        for i, p in enumerate(self.patterns):
            ...
            for t in range(self.t_learn + self.t_learnc + self.t_wash):
                ...
                if self.t_wash < t <= self.t_learnc + self.t_wash:
                    c = c + self.c_adapt_rate * (
                        (cphi - c * cphi) * cphi - math.pow(self.alpha, -2) * c
                    )
            ...
            self.all_train["r"][:, i * self.t_learn : (i + 1) * self.t_learn] = train[
                "r"
            ]
            self.all_train["old_r"][
                :, i * self.t_learn : (i + 1) * self.t_learn
            ] = train["old_r"]
            self.all_train["p"][:, i * self.t_learn : (i + 1) * self.t_learn] = train[
                "p"
            ]
            self.all_train["z"][:, i * self.t_learn : (i + 1) * self.t_learn] = train[
                "z"
            ]
            self.all_train["cphi"][
                :, i * self.t_learn : (i + 1) * self.t_learn
            ] = train["cphi"]
            self.all_train["old_cphi"][
                :, i * self.t_learn : (i + 1) * self.t_learn
            ] = train["old_cphi"]
        ...
        self.Z = (self.raw_Z @ np.diag(1.0 / norms_Z)) * mean_norms_Z
        ...
        txt = "Mean NRMSE per neuron for recomputing D = {0}".format(
            np.mean(self.NRMSE_load)
        )
        print(txt)
    ...
    def recall(self, t_washout=200, t_recall=200):
        ...
        for i in range(self.n_patts):
            ...
            cphi = 0.5 * np.random.randn(self.M)
            r = 0.1 * np.random.randn(self.N)
    ...
    def binocular(self, t_run=4000):
        ...
        noise_level = np.std(self.all_train["p"]) / self.SNR
        ...
        for t in range(t_run):
            ...
            y1mean = self.trust_smooth_rate * y1mean + (1 - self.trust_smooth_rate) * y1
            y1var = (
                self.trust_smooth_rate * y1var
                + (1 - self.trust_smooth_rate) * (y1 - y1mean) ** 2
            )
            unexplained1 = inaut - inext
            discrepancy1 = (
                self.trust_smooth_rate * discrepancy1
                + (1 - self.trust_smooth_rate) * unexplained1 ** 2 / y1var
            )
            ...
            r2 = np.tanh(
                self.G @ cphi2
                + self.W_in * ((1 - trust12) * inext + trust12 * inaut)
                + self.W_bias
            )
            cphi2 = c2 * (self.F @ r2)
            y2 = self.W_out @ r2

            y2mean = self.trust_smooth_rate * y2mean + (1 - self.trust_smooth_rate) * y2
            y2var = (
                self.trust_smooth_rate * y2var
                + (1 - self.trust_smooth_rate) * (y2 - y2mean) ** 2
            )
            unexplained2 = inaut - inext
            discrepancy2 = (
                self.trust_smooth_rate * discrepancy2
                + (1 - self.trust_smooth_rate) * unexplained2 ** 2 / y2var
            )
            ...
            r3 = np.tanh(
                self.G @ cphi3
                + self.W_in * ((1 - trust23) * inext + trust23 * inaut)
                + self.W_bias
            )
            cphi3 = c3 * (self.F @ r3)
            y3 = self.W_out @ r3

            y3mean = self.trust_smooth_rate * y3mean + (1 - self.trust_smooth_rate) * y3
            y3var = (
                self.trust_smooth_rate * y3var
                + (1 - self.trust_smooth_rate) * (y3 - y3mean) ** 2
            )
            unexplained3 = inaut - inext
            discrepancy3 = (
                self.trust_smooth_rate * discrepancy3
                + (1 - self.trust_smooth_rate) * unexplained3 ** 2 / y3var
            )

            trust12 = 1 / (
                1 + (discrepancy2 / discrepancy1) ** self.trust_adapt_steepness12
            )
            trust23 = 1 / (
                1 + (discrepancy3 / discrepancy2) ** self.trust_adapt_steepness23
            )
            ...
            t1 = self.Z @ np.power(hypo1, 2).T
            d_hypo1 = 2 * (np.power(cphi1, 2) - t1).T @ self.Z @ np.diag(
                np.squeeze(hypo1)
            ) + self.drift * (0.5 - hypo1)
            hypo1 = hypo1 + self.hypo_adapt_rate * d_hypo1
            hypo1 = hypo1 / np.sum(hypo1)

            t2 = self.Z @ np.power(hypo2, 2).T
            d_hypo2 = 2 * (np.power(cphi2, 2) - t2).T @ self.Z @ np.diag(
                np.squeeze(hypo2)
            ) + self.drift * (0.5 - hypo2)
            hypo2 = hypo2 + self.hypo_adapt_rate * d_hypo2
            hypo2 = hypo2 / np.sum(hypo2)
            # page 129
            t3 = self.Z @ np.power(hypo3, 2).T
            d_hypo3 = 2 * (np.power(cphi3, 2) - t3).T @ self.Z @ np.diag(
                np.squeeze(hypo3)
            ) + self.drift * (0.5 - hypo3)
            hypo3 = hypo3 + self.hypo_adapt_rate * d_hypo3
