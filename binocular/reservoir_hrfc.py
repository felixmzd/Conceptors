...
class ReservoirHRFC:
    ...
    def denoise(self, t_run=4000):
        ...
        c1 = t1 / (t1 + np.power(self.alpha, -2.0))
        c2 = t2 / (t2 + np.power(self.alpha, -2.0))
        c3 = t3 / (t3 + np.power(self.alpha, -2.0))
        ...
        for i, p in zip(range(self.n_patts), self.patterns):
            ...
            for t in range(t_run):
                ...
                c1_int = c1_int + self.c_adapt_rate * (
                            (cphi1 - c1_int * cphi1) * cphi1 - np.power(self.alpha, -2.0) * c1_int)
                ...
                c2_int = c2_int + self.c_adapt_rate * (
                            (cphi2 - c2_int * cphi2) * cphi2 - np.power(self.alpha, -2.0) * c2_int)
